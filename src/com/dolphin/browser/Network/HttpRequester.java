...
public class HttpRequester {
    ...
    public static final String TLSv1_2 = SSLSocketFactory.TLS + "v1.2";
    ...
    static final SSLContext createSSLConext() throws NoSuchAlgorithmException {
        SSLContext context;
        try {
    ...
    static final SSLSocketFactory createSSLSocketFactory() {
        try {
            return new SSLSocketFactory(TLSv1_2, null, null, null, null, null);
        } catch (Exception e) {
    ...
    public final HttpRequestResult request() throws IOException {
        return request(false);
    }

    public HttpRequestResult request(boolean useConnectionManager) throws IOException {
        final HttpClient client = createHttpClient(useConnectionManager, mData.mCheckSSLCertificate);
        HttpRequestBase request;

        if (mData.mEntity != null || mData.mMethod == HttpPost.METHOD_NAME) {
            request = new HttpPost(mData.mUrl);
            // Disable the f**king expectation
            request.getParams().setBooleanParameter(CoreProtocolPNames.USE_EXPECT_CONTINUE, false);
            ((HttpPost) request).setEntity(mData.mEntity);
        } else {
            request = new HttpGet(mData.mUrl);
        }
