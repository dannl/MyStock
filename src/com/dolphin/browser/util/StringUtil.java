...
public class StringUtil {
    ...
    /**
     * 采用二分法在升序String数组中查找以prefix为前缀的最小关键词, 查找过程为case insensitive
     *
     * @param ascendingKeyArray 按字母顺序升序排列的一组关键词
     * @param prefix 前缀字符串
     * @return 匹配成功则返回以prefix为前缀的最小关键词，失败返回null
     */
    public static String findMinStringByPrefix(String[] ascendingStringArray, String prefix) {

        if (ascendingStringArray == null || ascendingStringArray.length == 0 || TextUtils.isEmpty(prefix)) {
            return null;
        }

        int start = 0;
        int end = ascendingStringArray.length - 1;
        int mid = 0;

        if (ascendingStringArray[end].compareToIgnoreCase(prefix) < 0) {
            return null;
        }

        if (ascendingStringArray[start].compareToIgnoreCase(prefix) < 0) {

            while (start < end) {
                mid = start + (end - start) / 2;
                if (ascendingStringArray[mid].compareToIgnoreCase(prefix) >= 0) {
                    end = mid;
                } else {
                    start = mid + 1;
                }
        ...
        if (ascendingStringArray[start].toLowerCase().startsWith(prefix.toLowerCase())) {
            return ascendingStringArray[start];
        } else {
            return null;
        }
    }
